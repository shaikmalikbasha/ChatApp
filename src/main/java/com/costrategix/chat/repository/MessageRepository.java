...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT new com.costrategix.chat.dto.MessageHistoryDto(m.id, m.subject, m.content, m.fromId, m.threadId, mr.isRead, ma.fileName) FROM Message m, MessageRecipients mr, MessageAttachment ma WHERE m.fromId = :userId AND m.id = mr.messageId AND m.id = ma.messageId AND m.threadId IS NULL")
    ...
    @Query("SELECT new com.costrategix.chat.dto.MessageHistoryDto(m.id, m.subject, m.content, m.fromId, m.threadId, mr.isRead, ma.fileName) FROM Message m, MessageRecipients mr, MessageAttachment ma WHERE mr.recipientId = :userId AND m.id = mr.messageId AND m.id = ma.messageId AND m.threadId IS NULL")
